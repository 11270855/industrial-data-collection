Sub CreateEnergyManagementPresentation()
    '智能制造能源管理系统 - 自动生成PPT
    '使用方法：
    '1. 打开PowerPoint
    ...
    With pptSlide.Background.Fill
        .Visible = msoTrue
        .ForeColor.RGB = RGB(30, 58, 138) ' 深蓝色
        .BackColor.RGB = RGB(59, 130, 246) ' 浅蓝色
    ...
    With pptSlide.Shapes.Placeholders(2).TextFrame.TextRange
        .Text = "✓ 实时监控：设备能耗状态一目了然" & vbCrLf & _
                ...
                "✓ 成本降低：帮助企业节能10-20%"
        .Font.Size = 28
        .Font.Color.RGB = RGB(22, 163, 74) ' 绿色
    End With
    ...
    Set tbl = pptSlide.Shapes.AddTable(5, 2, 150, 180, 420, 200)
    
    With tbl.Table
        .Cell(1, 1).Shape.TextFrame.TextRange.Text = "性能指标"
        .Cell(1, 2).Shape.TextFrame.TextRange.Text = "数值"
        .Cell(2, 1).Shape.TextFrame.TextRange.Text = "数据采集频率"
        .Cell(2, 2).Shape.TextFrame.TextRange.Text = "1秒/次"
        .Cell(3, 1).Shape.TextFrame.TextRange.Text = "页面响应时间"
        .Cell(3, 2).Shape.TextFrame.TextRange.Text = "< 200ms"
        .Cell(4, 1).Shape.TextFrame.TextRange.Text = "并发用户数"
        .Cell(4, 2).Shape.TextFrame.TextRange.Text = "100+"
        .Cell(5, 1).Shape.TextFrame.TextRange.Text = "数据存储"
        .Cell(5, 2).Shape.TextFrame.TextRange.Text = "百万级记录"
        
        ' 设置表格样式
        For i = 1 To 5
            For j = 1 To 2
                .Cell(i, j).Shape.TextFrame.TextRange.Font.Size = 18
                .Cell(i, j).Shape.TextFrame.TextRange.ParagraphFormat.Alignment = ppAlignCenter
                If i = 1 Then
                    .Cell(i, j).Shape.Fill.ForeColor.RGB = RGB(30, 58, 138)
                    .Cell(i, j).Shape.TextFrame.TextRange.Font.Color.RGB = RGB(255, 255, 255)
                    .Cell(i, j).Shape.TextFrame.TextRange.Font.Bold = msoTrue
                End If
            Next j
        Next i
    End With
    ...
    ' 应用设计主题（可选，如果路径不存在会跳过）
    On Error Resume Next
    Dim themePath As String
    ' 尝试多个可能的主题路径
    themePath = Environ("ProgramFiles") & "\Microsoft Office\root\Document Themes 16\Ion.thmx"
    If Dir(themePath) <> "" Then
        pptPres.ApplyTheme themePath
    Else
        ' 如果找不到主题文件，使用内置设计理念
        pptPres.Designs(1).SlideMaster.ColorScheme.Colors(ppBackground).RGB = RGB(255, 255, 255)
    End If
    On Error GoTo 0
